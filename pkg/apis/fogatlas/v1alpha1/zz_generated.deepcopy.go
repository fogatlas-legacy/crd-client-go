...
func (in *FedFAAppChunk) DeepCopyInto(out *FedFAAppChunk) {
	*out = *in
	in.FederatedFADepl.DeepCopyInto(&out.FederatedFADepl)
	return
}
...
func (in *FedFAAppSpec) DeepCopyInto(out *FedFAAppSpec) {
	*out = *in
	if in.ApplicationChunks != nil {
		in, out := &in.ApplicationChunks, &out.ApplicationChunks
		*out = make([]*FedFAAppChunk, len(*in))
		for i := range *in {
			if (*in)[i] != nil {
				in, out := &(*in)[i], &(*out)[i]
				*out = new(FedFAAppChunk)
